...
def isValid(matrix, visited, i, j, s_i, s_j):
    ...
    if (matrix[i][j] == "E" and matrix[s_i][s_j] not in ("y", "z")):
        return False
    if (matrix[s_i][s_j] == "S" and matrix[i][j] in ("a", "b")) or (
            matrix[i][j] == "E" and matrix[s_i][s_j] in ("y", "z")) or ord(matrix[i][j]) - ord(
            matrix[s_i][s_j]) <= 1:
        return True
    return False
...
def shortest_path(matrix):
    ...
    while len(q) != 0:
        ...
        if isValid(matrix, vis, s.row + 1, s.col, s.row, s.col) and not vis[s.row + 1][s.col]:
            if matrix[s.row + 1][s.col] == "a" and ord(matrix[s.row + 1][s.col]) - ord(matrix[s.row ][s.col]) >=0 :
                q.append(Item(s.row + 1, s.col, s.dist )) #dont update distance
            else :
        ...
        if isValid(matrix, vis, s.row, s.col - 1, s.row, s.col):
            if matrix[s.row ][s.col-1] == "a"and ord(matrix[s.row][s.col-1]) - ord(matrix[s.row ][s.col]) >=0 :
                q.append(Item(s.row, s.col-1, s.dist )) #dont update distance
            else :
